...
import type { AgsConfig, RoleConfig, ToolType } from '../types.js';
...
export function loadConfig(cwd: string = process.cwd()): AgsConfig {
  ...
  if (!existsSync(configPath)) {
    throw new Error(
      `No ags.yaml found in ${cwd}. Run "ags init" to set up this project.`
    );
  }
  const raw = readFileSync(configPath, 'utf8');
  const parsed = parse(raw) as AgsConfig;
  return applyDefaults(parsed);
}

export function saveConfig(config: AgsConfig, cwd: string = process.cwd()): void {
  const configPath = join(cwd, CONFIG_FILE);
...
function applyDefaults(config: AgsConfig): AgsConfig {
  return {
...
function mergeRoleConfig(role: string, override?: Partial<RoleConfig>): RoleConfig {
