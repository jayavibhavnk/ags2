...
export class Swarm extends EventEmitter {
  ...
  constructor(
    ...
    for (const [id, instance] of Object.entries(this.state.agents)) {
      ...
      agent.on('output', (line: AgentOutputLine) => {
  ...
  abort(): void {
    this.aborted = true;
    this.emit('event', { type: 'error', message: 'Swarm aborted by user' } satisfies SwarmEvent);
  }
  ...
  async run(): Promise<void> {
    ...
    this.emit('event', { type: 'phase_change', phase: 'coding' } satisfies SwarmEvent);
    ...
    const coderPromises = Array.from({ length: coderInstances }, (_, i) => {
      ...
      return this.runRole(id, {
        ...
        if (result) coderResults.push(result);
      });
    });
    ...
    if (this.config.workflow.autoReview !== false) {
      ...
      this.emit('event', { type: 'phase_change', phase: 'reviewing' } satisfies SwarmEvent);
    ...
    if (this.config.workflow.autoTest !== false) {
      ...
      this.emit('event', { type: 'phase_change', phase: 'testing' } satisfies SwarmEvent);
      ...
      if (testResult && !this.aborted) {
        ...
        if (!testReport.passed) {
          ...
          this.emit('event', { type: 'phase_change', phase: 'debugging' } satisfies SwarmEvent);
  ...
  private appendOutput(
    ...
    const line: AgentOutputLine = { timestamp: Date.now(), type, content };
  ...
  private makeCtx(): RolePromptContext {
    return {
  ...
  private buildAgentInstances(): AgentInstance[] {
    const instances: AgentInstance[] = [];

    const addRole = (
      id: string,
      roleId: AgentInstance['role'],
      cfg: typeof this.config.orchestrator
    ) => {
      instances.push({
        id,
        role: roleId,
  ...
  private extractVerdict(reviewOutput: string): { verdict: 'approved' | 'needs_changes' } {
    try {
      const match = reviewOutput.match(/\{[\s\S]*?"verdict"\s*:\s*"(approved|needs_changes)"[\s\S]*?\}/);
      if (match) {
        const parsed = JSON.parse(match[0]) as { verdict: 'approved' | 'needs_changes' };
        return parsed;
      }
    } catch {
      /* fall through */
  ...
  private extractTestReport(testOutput: string): { passed: boolean } {
    try {
    ...
    if (/all tests passed|✓|PASS/i.test(testOutput)) return { passed: true };
    if (/FAIL|failed|error/i.test(testOutput)) return { passed: false };
    return { passed: true };
  }
