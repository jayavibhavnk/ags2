...
export async function runInit(cwd: string = process.cwd()): Promise<void> {
  ...
  const basics = await prompt<{ name: string; description: string; techStack: string }>([
    {
      type: 'input',
      name: 'name',
  ...
  // Coder instances
  const coderSetup = await prompt<{ instances: string; worktrees: boolean }>([
    {
      type: 'select',
      name: 'instances',
  ...
  const tools = await prompt<{
    architectTool: ToolType;
    coderTool: ToolType;
    reviewerTool: ToolType;
    testerTool: ToolType;
    debuggerTool: ToolType;
  }>([
    {
      type: 'select',
      name: 'architectTool',
...
function getDefaultModel(tool: ToolType, role: string): string {
  const models: Record<ToolType, Record<string, string>> = {
    'claude-code': {
...
function printConfig(config: AgsConfig): void {
  ...
  for (const [name, role] of roles) {
    ...
    console.log(
      ...
      chalk.white(`${role.tool}`),
      chalk.dim(`(${role.model ?? 'default'})${instances}`)
    );
  }
  console.log();
}
