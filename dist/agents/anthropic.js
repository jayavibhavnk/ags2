    ...
    try {
        switch (name) {
            ...
            case 'edit_file': {
                ...
                writeFileSync(p, src.replace(old, input['new_string']), 'utf8');
...
export class AnthropicAgent extends BaseAgent {
    ...
    async run(opts) {
        const { prompt, systemPrompt, cwd = process.cwd(), maxTurns = 40, onOutput, } = opts;
        ...
        while (turns < maxTurns) {
            ...
            try {
                ...
                for await (const event of stream) {
                    if (event.type === 'content_block_start') {
                        ...
                        if (event.usage) {
                            ...
                            const inCost = event.usage.input_tokens ?? 0;
                            const outCost = event.usage.output_tokens ?? 0;
                ...
                for (const block of assistantContent) {
                    ...
                    const result = executeTool(block.name, block.input, cwd);
