...
export function runSync(cwd = process.cwd()) {
    let config;
    try {
    ...
    writeFileSync(claudeMdPath, generateClaudeMd(config.project.name, config.project.description), 'utf8');
    console.log(chalk.green('  ✓ Updated: CLAUDE.md'));
    // Overwrite AGENTS.md
    const agentsMdPath = join(cwd, 'AGENTS.md');
    writeFileSync(agentsMdPath, generateAgentsMd(config.project.name, config.project.description), 'utf8');
