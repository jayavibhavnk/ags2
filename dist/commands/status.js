...
export function runStatus(cwd = process.cwd()) {
    if (!configExists(cwd)) {
    ...
    console.log();
    console.log(chalk.bold.hex('#7C3AED')('  ags status') +
    ...
    const phaseColor = state.phase === 'done' ? '#10B981' :
        state.phase === 'failed' ? '#EF4444' :
            '#818CF8';
    console.log(chalk.dim('  Task:    '), chalk.white(state.userTask));
    console.log(chalk.dim('  Phase:   '), chalk.hex(phaseColor)(PHASE_LABELS[state.phase] ?? state.phase));
    console.log(chalk.dim('  Elapsed: '), chalk.white(`${elapsed}s`));
    console.log(chalk.dim('  Cost:    '), chalk.white(`$${state.totalCost.toFixed(4)}`));
    if (state.revisionLoops > 0) {
        console.log(chalk.dim('  Revisions:'), chalk.white(`${state.revisionLoops}`));
    }
    console.log();
    console.log(chalk.dim('  Agents:'));
    for (const agent of Object.values(state.agents)) {
        const icon = STATUS_ICONS[agent.status] ?? '?';
        const statusColor = agent.status === 'done' ? '#10B981' :
        ...
        console.log(`    ${chalk.hex(statusColor)(icon)} ${chalk.white(name.padEnd(16))}`, chalk.dim(agent.tool), cost);
    }
    if (state.lastError) {
        console.log();
