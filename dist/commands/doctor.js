...
export async function runDoctor() {
    ...
    for (const tool of TOOLS) {
        ...
        if (result.installed) {
            console.log(chalk.green('  ✓'), chalk.white(tool.name.padEnd(16)), chalk.dim(`v${result.version ?? '?'}`));
        }
        else {
            allOk = false;
            console.log(chalk.red('  ✗'), chalk.white(tool.name.padEnd(16)), chalk.dim('not found'));
            console.log(chalk.dim(`       install: ${tool.installHint}`));
        }
    }
    console.log();
    ...
    if (nodeMajor >= 20) {
        ...
        console.log(chalk.yellow('  ⚠'), chalk.white('Node.js'.padEnd(16)), chalk.dim(`${nodeVersion} (>=20 recommended)`));
    }
    console.log();
    if (allOk) {
